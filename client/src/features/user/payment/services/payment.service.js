...
export const capturePayPalOrder = async ({ orderID, lockId, couponCode }) => {
  const { data } = await api.post("/user/checkout/payment/paypal/capture", {
    orderID,
    lockId,
    couponCode,
  });
...
export const createWalletOrder = async ({ lockId, couponCode }) => {
  const { data } = await api.post("/user/checkout/payment/wallet", {
    lockId,
    couponCode,
  });
